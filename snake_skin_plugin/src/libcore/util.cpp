...
Float ThinFilmTransmission(Float cos0, Float lambda, Float thickness, Float n0, Float n1, Float n2){
    //  -------------------------------------------------
    //      Establecemos la variable del coeficiente de refracción
    //  -------------------------------------------------
    Float I_t = 0;
    ...
    //
    //      Ley Snell:
    //          n0 * sin0 = n1 * sin1 = n2 * sin2
    //          sin1 = (n0 * sin0) / n1
    //          sin2 = (n1 * sin1) / n2
    //
    //      Razones trignonométricas:
    //          cos² + sin² = 1 ==> cos = sqrt(1 - sin²) ==> sin = sqrt(1 - cos²)
    //  -------------------------------------------------
    ...
    //  -------------------------------------------------
    //      Calculamos el ratio de conservación de energía
    //  -------------------------------------------------

    Float conservationRatio = (n2 * cos2) / (n0 * cos0);

    //  -------------------------------------------------
    //      Calculamos phi para el cambio de fase
    //  -------------------------------------------------
    Float d10 = (n1 >= n0) ? 0.0 : M_PI;
    Float d12 = (n1 >= n2) ? 0.0 : M_PI;
    Float delta = d10 + d12;

    Float phi = (2.0 * M_PI / lambda) * (2.0 * n1 * thickness * cos1) + delta;


    //  -------------------------------------------------
    //      Dado que tenemos luz s-polarizada y p-polarizada, lo que vamos a hacer es asumir
    //      que tenemos lo mismo de cada por lo que haremos una media de los resultados.
    //      Para su cálculo necesitamos los siguientes cálculos dadas las ecuaciones de Fresnell
    //      - B_sPolariced: ts(n0, n1, cos0, cos1) * ts(n1, n2, cos1, cos2)
    //      - B_pPolariced: tp(n0, n1, cos0, cos1) * tp(n1, n2, cos1, cos2)
    //      - a_sPolariced: rs(n1, n0, cos1, cos0) * rs(n1, n2, cos1, cos2)
    //      - a_pPolariced: rp(n1, n0, cos1, cos0) * rp(n1, n2, cos1, cos2)
    //  -------------------------------------------------

    Float B_sPolariced = ts(n0, n1, cos0, cos1) * ts(n1, n2, cos1, cos2);
    Float B_pPolariced = tp(n0, n1, cos0, cos1) * tp(n1, n2, cos1, cos2);
    Float a_sPolariced = rs(n1, n0, cos1, cos0) * rs(n1, n2, cos1, cos2);
    Float a_pPolariced = rp(n1, n0, cos1, cos0) * rp(n1, n2, cos1, cos2);

    // Una vez tenemos las piezas de la ecuación la calculamos para cada tipo de luz
    // y hacemos la media como se ha comentado.

    Float transmitted_s = pow(B_sPolariced, 2.0) / (pow(a_sPolariced, 2.0) - 2.0 * a_sPolariced * cos(phi) + 1.0);
    Float transmitted_p = pow(B_pPolariced, 2.0) / (pow(a_pPolariced, 2.0) - 2.0 * a_pPolariced * cos(phi) + 1.0);
    Float transmitted = (transmitted_s + transmitted_p) / 2.0;

    //  -------------------------------------------------
    //      Ya tenemos ambas partes de la ecuación solo hay que resolverla en la variable
    //      I_t que hemos preparado al principio
    //  ------------------------------------------------- 

    I_t = conservationRatio * transmitted;
    
    return std::min(1.0, std::max(0.0, 0.0 + I_t));
}

Float ThinFilmReflectance(Float cos0, Float lambda, Float thickness, Float n0, Float n1, Float n2){

    Float I_t = ThinFilmTransmission(cos0, lambda, thickness, n0, n1, n2);
    
    //  -------------------------------------------------
    //      Teniendo en cuenta la conservación de energía I_t + I_r = 1 calculamos I_r
    //  -------------------------------------------------
    return std::min(1.0, std::max(0.0, 1.0 - I_t));
}

Float ThinFilmTransmissionExt(Float cos0, Float &cosThetaT_, Float lambda, Float thickness, Float n0, Float n1, Float n2){
//  -------------------------------------------------
    //      Establecemos la variable del coeficiente de refracción
    //  -------------------------------------------------
    Float I_t = 0;
    ...
    float cos2 = sqrt(1.0 - sin2);
    cosThetaT_ = cos2;
    cosThetaT_ = (cos0 > 0) ? -cosThetaT_ : cosThetaT_;
...
Float ThinFilmReflectanceExt(Float cos0, Float &cosThetaT_, Float lambda, Float thickness, Float n0, Float n1, Float n2){
    
    Float I_t = ThinFilmTransmissionExt(cos0, cosThetaT_, lambda, thickness, n0, n1, n2);
