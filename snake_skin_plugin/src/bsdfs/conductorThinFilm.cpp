 ...
 * \end{table}
 */
class SmoothConductorThinFilm : public BSDF {
public:
    SmoothConductorThinFilm(const Properties &props) : BSDF(props) {
        ...
        mediumIOR = lookupIOR(props, "mediumIOR", "air");
        intIOR = lookupIOR(props, "intIOR", "air");*/
        ...
        m_eta0 = props.getSpectrum("eta", intEta) / extIOR;
        m_k   = props.getSpectrum("k", intK) / extIOR;
    }

    SmoothConductorThinFilm(Stream *stream, InstanceManager *manager)
            : BSDF(stream, manager) {
        m_specularReflectance = static_cast<Texture *>(manager->getInstance(stream));
        m_eta0 = Spectrum(stream);
        m_k = Spectrum(stream);
    ...
    Spectrum eval(const BSDFSamplingRecord &bRec, EMeasure measure) const {
        ...
        float r = ThinFilmReflectance(Frame::cosTheta(bRec.wi), 650.0, m_thickness, extIOR, mediumIOR, intIOR);
        float g = ThinFilmReflectance(Frame::cosTheta(bRec.wi), 510.0, m_thickness, extIOR, mediumIOR, intIOR);
        float b = ThinFilmReflectance(Frame::cosTheta(bRec.wi), 475.0, m_thickness, extIOR, mediumIOR, intIOR);
    ...
    Spectrum sample(BSDFSamplingRecord &bRec, const Point2 &sample) const {
        ...
        return m_specularReflectance->eval(bRec.its) * (spec/F);
    }
    ...
    std::string toString() const {
        ...
        oss << "SmoothConductorThinFilm[" << endl
            << "  id = \"" << getID() << "\"," << endl
            << "  eta = " << m_eta0.toString() << "," << endl
            << "  k = " << m_k.toString() << "," << endl
...
*/
class SmoothConductorThinFilmShader : public Shader {
public:
    SmoothConductorThinFilmShader(Renderer *renderer, const Texture *specularReflectance,
...
Shader *SmoothConductorThinFilm::createShader(Renderer *renderer) const {
    return new SmoothConductorThinFilmShader(renderer,
        m_specularReflectance.get(), m_eta0, m_k);
}

MTS_IMPLEMENT_CLASS(SmoothConductorThinFilmShader, false, Shader)
MTS_IMPLEMENT_CLASS_S(SmoothConductorThinFilm, false, BSDF)
MTS_EXPORT_PLUGIN(SmoothConductorThinFilm, "Smooth conductor thin film");
MTS_NAMESPACE_END
